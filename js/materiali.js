const materialSearchInput = document.querySelector('.catalog-search-input');
const materialSearchWrap = document.querySelector('.catalog-search-wrap');
const materialCards = Array.from(document.querySelectorAll('.catalog-card'));
const materialSortButton = document.querySelector('.toolbar-sort-btn');
const materialFilterLinks = Array.from(document.querySelectorAll('.filter-list a'));
const materialViewButtons = Array.from(document.querySelectorAll('.toolbar-view-btn'));
const materialGrid = document.querySelector('.catalog-grid');
...
function applyViewMode() {
  ...
  materialGrid.classList.toggle('catalog-grid--list', viewMode === 'list');
  materialViewButtons.forEach((button) => {
...
function renderCards() {
  if (!materialCards.length || !materialGrid) return;

  const visibleCards = materialCards.filter(matchesFilters);
...
function updateSortLabel() {
  ...
  const label = materialSortButton.querySelector('span');
...
function clearFilters() {
  ...
  materialFilterLinks.forEach((link) => link.classList.remove('is-active'));
  updateSortLabel();
  renderCards();
}

if (materialSearchInput && materialSearchWrap) {
  ...
  materialSearchWrap.appendChild(clearButton);

  materialSearchInput.addEventListener('input', renderCards);
...
materialFilterLinks.forEach((link) => {
  link.addEventListener('click', (event) => {
    event.preventDefault();

    const label = (link.textContent || '').trim().toLowerCase();
    const nextCategory = label.includes('legno') ? 'legno'
      : label.includes('acril') ? 'acrilico'
        : label.includes('vinile') ? 'vinile'
          : label.includes('varie') ? 'varie'
            : '';

    activeCategoryFilter = activeCategoryFilter === nextCategory ? '' : nextCategory;

    materialFilterLinks.forEach((item) => item.classList.remove('is-active'));
...
if (materialSortButton) {
  materialSortButton.addEventListener('click', () => {
