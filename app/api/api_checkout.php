...
try {
    ...
    foreach ($items as $item) {
        ...
        $lineItems[] = [
            ...
            'description' => $printer->getSubtitle(),
            'unit_amount' => (int) round($printer->getPrice() * 100),
            'quantity' => $item->getQuantity(),
            'image' => $printer->getImagePath() !== '' ? (stripos($printer->getImagePath(), 'http') === 0 ? $printer->getImagePath() : (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . '/' . ltrim($printer->getImagePath(), '/')) : null,
        ];
    }
